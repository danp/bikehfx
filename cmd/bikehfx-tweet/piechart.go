...
func NewPieChart(vs plotter.Valuer) (*PieChart, error) {
	...
	pie.Labels.TextStyle.Font = font.From(plotter.DefaultFont, plotter.DefaultFontSize)
	pie.Labels.TextStyle.Handler = plot.DefaultTextHandler
	return pie, nil
}
...
func (p *PieChart) Plot(c draw.Canvas, plt *plot.Plot) {
	...
	for i, v := range p.Values {
		...
		c.SetLineWidth(p.LineStyle.Width)
		c.Stroke(path)
...
func sumValues(vs plotter.Values) float64 {
	sum := 0.
	for _, v := range vs {
		sum += v
	}
...
func rotatePoint(pt, ref vg.Point, angle float64) vg.Point {
	return vg.Point{
		X: vg.Length(float64(pt.X-ref.X)*math.Cos(angle)-
			float64(pt.Y-ref.Y)*math.Sin(angle)) +
			ref.X,
		Y: vg.Length(float64(pt.Y-ref.Y)*math.Cos(angle)+
			float64(pt.X-ref.X)*math.Sin(angle)) +
			ref.Y,
	}
}
